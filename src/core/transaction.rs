...
impl Transaction {
    ...
    pub fn new_deploy(
        ...
        Self {
            from,
            payload: TxPayload::ContractDeploy {
                wasm_code,
                init_args,
            },
            nonce,
    ...
    pub fn sign(&mut self, private_key: &SecretKey) -> Result<(), String> {
        ...
        let message =
            Message::from_digest_slice(&message).map_err(|e| format!("Invalid message: {}", e))?;
    ...
    /// Get the recipient address if this is a transfer transaction
    pub fn to(&self) -> Option<Address> {
        match &self.payload {
            TxPayload::Transfer { to, .. } => Some(*to),
            TxPayload::Coinbase { to, .. } => Some(*to),
            _ => None,
        }
    }
    ...
    pub fn validate_basic(&self) -> Result<(), String> {
        ...
        if self.gas_limit == 0 {
            if !matches!(self.payload, TxPayload::Coinbase { .. }) {
    ...
    pub fn get_fee(&self) -> Option<u64> {
        Some(self.gas_limit * self.gas_price)
    }

    /// Create coinbase transaction for mining reward
    pub fn new_coinbase(miner_address: Address, reward: u64, height: u64, blue_score: u64) -> Self {
        Self {
            from: [0u8; 20], // Burn address for coinbase
