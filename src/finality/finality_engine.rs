...
struct IntervalNode {
    ...
    reachability_cache: RwLock<HashMap<BlockHash, bool>>,
}
...
#[allow(dead_code)]
#[derive(Debug, Clone)]
pub struct VirtualSelectedParent {
    /// The VSP block hash
...
impl FinalityEngine {
    ...
    fn build_interval_tree(
        &self,
        ordering: &[BlockHash],
        dag: &crate::dag::blockdag::BlockDAG,
    ) -> Result<(), String> {
        if ordering.is_empty() {
            return Ok(());
        }
        ...
        for (index, block_hash) in ordering.iter().enumerate() {
            ...
            nodes.push(IntervalNode {
                ...
                left: None,
                right: None,
                reachability_cache: RwLock::new(HashMap::new()),
            });
        }
    ...
    fn build_balanced_tree(&self, mut nodes: Vec<IntervalNode>) -> Box<IntervalNode> {
        ...
        let (left_nodes, right_nodes) = nodes
            .into_iter()
    ...
    pub fn can_reach(&self, from: &BlockHash, to: &BlockHash) -> Result<bool, String> {
        ...
        if let Some(node) = self.find_node(&tree.as_ref().unwrap(), from) {
            if let Some(&cached) = node.reachability_cache.read().get(to) {
                return Ok(cached);
            }
        }
        ...
        // Cache result
        if let Some(node) = self.find_node(&tree.as_ref().unwrap(), from) {
            node.reachability_cache.write().insert(*to, can_reach);
            // Limit cache size
            if node.reachability_cache.read().len() > self.max_cache_size / 100 {
                node.reachability_cache.write().clear();
            }
        }
    ...
    fn find_node<'a>(
        &self,
        tree: &'a Box<IntervalNode>,
        target: &BlockHash,
    ) -> Option<&'a IntervalNode> {
    ...
    fn compute_virtual_selected_parents(
        &self,
        dag: &crate::dag::blockdag::BlockDAG,
    ) -> Result<(), String> {
    ...
    fn compute_single_vsp(
        &self,
        block_hash: &BlockHash,
        dag: &crate::dag::blockdag::BlockDAG,
    ) -> Result<VirtualSelectedParent, String> {
        // Get block's parents
        let parents = dag
            .get_block_parents(block_hash)
    ...
    fn calculate_score_based_finality(
        &self,
        dag: &crate::dag::blockdag::BlockDAG,
    ) -> Result<(), String> {
    ...
    fn calculate_finality_threshold(
        &self,
        head_score: u64,
        _dag: &crate::dag::blockdag::BlockDAG,
    ) -> Result<u64, String> {
    ...
    pub fn get_anticone(
        ...
        block_hash: &BlockHash,
        dag: &crate::dag::blockdag::BlockDAG,
    ) -> Result<HashSet<BlockHash>, String> {
        // Check cache first
    ...
    pub fn get_finality_confidence(&self, block_hash: &BlockHash) -> f64 {
        let scores = self.block_scores.read();
        let _heights = self.block_heights.read();
    ...
    pub fn get_virtual_selected_parent(
        &self,
        block_hash: &BlockHash,
    ) -> Option<VirtualSelectedParent> {
    ...
    pub fn get_stats(&self) -> FinalityStats {
        ...
        let total_finalized = finalized.values().map(|set| set.len()).sum();
    ...
    fn create_test_blocks() -> Vec<Block> {
        ...
        for i in 1u8..=10u8 {
            let parents = if i == 1 {
                ...
                vec![blocks[(i - 2) as usize].hash]
            };
            let block = Block::new(parents, 1000 + i as u64, vec![], 0, 0, 0, [0; 20], [0; 32]);
            blocks.push(block);
        }
