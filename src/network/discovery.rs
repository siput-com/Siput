...
/// Network service discovery manager untuk Siput-speed
...
impl DiscoveryManager {
    ...
    pub fn new(local_peer_id: PeerId) -> Self {
        ...
        dns_seeds.push(DnsSeed {
            hostname: "seed.siput.net".to_string(),
            port: 8333,
        });
        dns_seeds.push(DnsSeed {
            hostname: "seed2.siput.net".to_string(),
            port: 8333,
        });
    ...
    pub fn add_dns_seed(&self, seed: DnsSeed) {
        ...
        info!(
            "Added DNS seed: {}:{}",
            seed_clone.hostname, seed_clone.port
        );
    }
    ...
    /// Add known peer
    pub fn add_known_peer(&self, peer_id: PeerId, address: Option<Multiaddr>) {
        ...
        if !peers.contains_key(&peer_id) {
            ...
            peers.insert(peer_id, peer_data);
            info!("Added known peer: {}", peer_id);
        }
    }
    ...
    async fn query_dns_seed(&self, seed: &DnsSeed) -> Result<Vec<PeerId>, String> {
        ...
        let mock_peers = vec![PeerId::random(), PeerId::random(), PeerId::random()];
    ...
    pub fn get_peers_to_connect(&self) -> Vec<PeerId> {
        ...
        peers
            ...
            .filter(|(peer_id, peer_data)| {
                !banned.contains(peer_id) && !peer_data.connected && peer_data.should_connect()
            })
            .take(self.max_peers)
    ...
    pub fn parse_multiaddr(addr: &str) -> Option<String> {
        if let Some(p2p_pos) = addr.find("/p2p/") {
            let peer_id_part = &addr[p2p_pos + 5..];
...
mod tests {
    ...
    fn test_known_peers() {
        ...
        dm.add_known_peer(peer1, None);
        dm.add_known_peer(peer2, None);
    ...
    fn test_multiaddr_parsing() {
        ...
        assert_eq!(
            peer_id,
            Some("QmAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA".to_string())
        );
    }
