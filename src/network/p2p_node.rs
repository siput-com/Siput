...
use crate::network::{GossipProtocol, NetworkInterface, NetworkMessage, PeerManager, SyncManager};
...
impl P2PNode {
    ...
    pub async fn new(
        ...
        let floodsub_topic = libp2p::floodsub::Topic::new("siput-blocks");
    ...
    async fn handle_network_message(&self, peer: PeerId, message: NetworkMessage) {
        match message {
            NetworkMessage::NewBlock(block) => {
                ...
                if let Err(e) = self
                    .gossip
                    .handle_gossip_message(NetworkMessage::NewBlock(block))
                    .await
                {
            ...
            NetworkMessage::NewTransaction(tx) => {
                ...
                if let Err(e) = self
                    .gossip
                    .handle_gossip_message(NetworkMessage::NewTransaction(tx))
                    .await
                {
            ...
            NetworkMessage::GetHeaders { from, max } => {
                let _ = self
                    .sync_manager
                    .handle_message(peer.clone(), NetworkMessage::GetHeaders { from, max })
                    .await;
            }
            ...
            NetworkMessage::GetBlocks(hashes) => {
                let _ = self
                    .sync_manager
                    .handle_message(peer.clone(), NetworkMessage::GetBlocks(hashes))
                    .await;
            }
            ...
            NetworkMessage::RequestState => {
                let _ = self
                    .sync_manager
                    .handle_message(peer.clone(), NetworkMessage::RequestState)
                    .await;
            }
            NetworkMessage::StateSnapshot(data) => {
                // deserialize and verify
                if let Ok(state) =
                    bincode::deserialize::<crate::state::state_manager::StateManager>(&data)
                {
                    ...
                    if root == local_root {
                        ...
                        tracing::warn!(
                            "State snapshot root mismatch {} vs {}",
                            hex::encode(root),
    ...
    async fn send_message_to_peer(&mut self, _peer: PeerId, message: NetworkMessage) {
        ...
        let topic = libp2p::floodsub::Topic::new("siput-blocks");
