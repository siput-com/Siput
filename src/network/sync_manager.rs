...
impl SyncManager {
    ...
    pub async fn handle_headers(
        ...
        if !missing.is_empty() {
            tracing::info!(
                count = missing.len(),
                "Detected missing blocks from headers, requesting..."
            );
    ...
    pub async fn handle_message(&self, peer: PeerId, msg: NetworkMessage) -> Result<(), String> {
        match msg {
            NetworkMessage::GetHeaders { from, max } => {
                ...
                let _ = self
                    .message_sender
                    .send((peer, NetworkMessage::Headers(headers)));
                Ok(())
            }
            ...
            NetworkMessage::GetBlocks(hashes) => {
                ...
                let _ = self
                    .message_sender
                    .send((peer, NetworkMessage::Blocks(blocks)));
                Ok(())
            }
            NetworkMessage::RequestState => {
                ...
                let _ = self
                    .message_sender
                    .send((peer, NetworkMessage::StateSnapshot(snapshot)));
                Ok(())
            }
            _ => Ok(()),
        }
