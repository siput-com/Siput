...
impl StateSyncManager {
    ...
    pub async fn handle_state_snapshot(&self, snapshot: StateSnapshot) -> Result<(), String> {
        ...
        *self.sync_in_progress.write() = false;
        ...
        tracing::info!(
            "State sync completed at block height {}",
            snapshot.block_height
        );
        Ok(())
    }
...
mod tests {
    ...
    fn test_snapshot_restore() {
        let state = StateManager::new();
        let snapshot =
            StateSnapshot::from_state_manager(&state, 50, [1; 32]).expect("Should create snapshot");

        let restored = snapshot
            .restore_to_state_manager()
