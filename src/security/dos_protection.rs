...
impl DosProtection {
    ...
    fn handle_violation(&self, ip: IpAddr, state: &mut PeerState, reason: &str) {
        state.violations += 1;
        ...
        warn!(
            "DoS violation from {}: {} (violation #{})",
            ip, reason, state.violations
        );
        ...
        if state.violations >= 3 {
            ...
            warn!(
                "Banned {} for {} seconds due to repeated violations",
    ...
    fn start_cleanup_task(&self) {
        ...
        thread::spawn(move || {
            loop {
                thread::sleep(Duration::from_secs(config.cleanup_interval_seconds));
                ...
                if states_write.len() > config.max_tracked_ips {
                    ...
                    let mut entries: Vec<(IpAddr, Instant)> = states_write
                        .iter()
                        .map(|(ip, state)| (*ip, state.last_activity))
                        .collect();
...
mod tests {
    ...
    async fn test_connection_limits() {
        ...
        assert!(matches!(
            dos.check_connection_allowed(ip),
            Err(DosError::ConnectionLimitExceeded)
        ));
    }
