...
impl BatchSignatureVerifier {
    ...
    pub fn verify_transaction_batch(
        ...
        let verification_data: Vec<_> = transactions
            .iter()
            .enumerate()
            .map(|(idx, tx)| {
                let message = Message::from_digest(tx.hash());
                let recovered_pubkey = self
                    .recover_public_key_from_tx(tx, &message)
                ...
                if expected_address != tx.from {
                    return Err(format!(
                        "Transaction {}: signature does not match from address",
                        idx
                    ));
                }
        ...
        debug!(
            "Batch verified {} transactions: {} valid, {} invalid in {}ms",
            transactions.len(),
            valid_count,
            invalid_count,
            total_time
    ...
    fn recover_public_key_from_tx(
        ...
        let _sig = secp256k1::ecdsa::Signature::from_compact(sig_bytes)
...
impl VerificationStats {
    ...
    pub fn avg_time_per_tx_ms(&self) -> f64 {
        if self.total_verifications == 0 {
            ...
            let avg = self.total_time_ms as f64 / self.total_verifications as f64;
            if avg == 0.0 { 0.1 } else { avg }
        }
    }
