...
impl SpamDetector {
    ...
    pub fn check_spam(&self, tx: &Transaction, ip: std::net::IpAddr) -> SpamResult {
        ...
        let recent_count = activity
            ...
            .iter()
            .filter(|&&time| {
                now.duration_since(time) < Duration::from_secs(self.config.burst_window_seconds)
            })
            .count();
        ...
        let tx_pattern = TransactionPattern {
            ...
            amount: match &tx.payload {
                ...
                TxPayload::ContractDeploy { .. } => 0,
                TxPayload::ContractCall { .. } => 0,
            },
    ...
    fn calculate_similarity(&self, a: &TransactionPattern, b: &TransactionPattern) -> f64 {
        ...
        if a.payload_size == b.payload_size {
            ...
            let len_diff = (a.payload_size as f64 - b.payload_size as f64).abs()
                / (a.payload_size.max(b.payload_size) as f64);
...
impl FuzzTester {
    ...
    fn detect_edge_case(&self, tx: &Transaction) -> Option<String> {
        let (amount, to) = match &tx.payload {
            ...
            TxPayload::Coinbase { amount, to, .. } => (*amount, *to),
            TxPayload::ContractDeploy { wasm_code: _, .. } => (0, [0u8; 20]),
            TxPayload::ContractCall {
                contract_address, ..
            } => (0, *contract_address),
        };
...
mod tests {
    ...
    fn test_spam_detection_rate_limit() {
        ...
        assert!(matches!(
            detector.check_spam(&tx, ip),
            SpamResult::Blocked(SpamReason::RateLimitExceeded)
        ));
    }
