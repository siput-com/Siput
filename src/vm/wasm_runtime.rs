...
/// Runtime interface for dependency injection and loose coupling with execution.
pub trait WasmRuntimeInterface: Send + Sync {
...
impl WasmRuntimeInterface for WasmRuntime {
    fn instantiate_contract(
        &self,
        bytecode: &[u8],
        state: RuntimeState,
        ...
        WasmRuntime::instantiate_contract(self, bytecode, state, gas_limit)
    }
    ...
    fn call_function(
        ...
        WasmRuntime::call_function(self, store, instance, func_name, args)
    }
}
...
impl WasmRuntime {
    ...
    pub fn instantiate_contract(
        ...
        // provide fuel for Wasmtime execution so even no-op modules run
...
mod tests {
    ...
    #[test]
    fn test_trivial_wasm_execution() {
        let wasm = wat::parse_str("(module (func (export \"foo\") (nop)))").unwrap();
        ...
        let state = RuntimeState {
            ...
            block_height: 0,
            timestamp: 0,
            storage: Box::new(crate::contracts::contract_storage::ContractStorage::new("test_storage_wasm_runtime")),
            gas_meter: crate::vm::gas_meter::GasMeter::new(1000),
            memory_limiter: None,
        };
        let (mut store, instance) = runtime.instantiate_contract(&wasm, state, 1000).unwrap();
        let result = runtime.call_function(&mut store, &instance, "foo", &[]);
        assert!(result.is_ok(), "WASM call failed: {:?}", result.err());
    }
}
