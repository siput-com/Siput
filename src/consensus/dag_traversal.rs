...
impl DAGTraversal {
    ...
    pub fn get_path(dag: &BlockDAG, from: &BlockHash, to: &BlockHash) -> Option<Vec<BlockHash>> {
    ...
    pub fn get_common_ancestors(dag: &BlockDAG, hashes: &[BlockHash]) -> HashSet<BlockHash> {
        if hashes.is_empty() {
            return HashSet::new();
        }
