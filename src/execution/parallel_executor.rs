...
use crate::core::transaction::{Address, Transaction, TxPayload};
use crate::state::state_manager::StateManager;
use crate::state::state_tree::Account;
use crate::vm::gas_meter::GasMeter;
use crate::vm::{RuntimeState, WasmStorage};
use rayon::prelude::*;
...
pub fn analyze_access(tx: &Transaction) -> TxAccessSet {
    ...
    match &tx.payload {
        ...
        TxPayload::Coinbase { to, .. } => {
            // Coinbase should not be in parallel execution queue
            writes.insert(StateKey::Account(*to));
        }
...
impl ContractStorageOverlay {
    pub fn new(base: Arc<ContractStorage>) -> Self {
        ContractStorageOverlay {
            inner: Arc::new(std::sync::Mutex::new(ContractStorageOverlayInner {
                base,
...
impl WasmStorage for ContractStorageOverlay {
    ...
    fn box_clone(&self) -> Box<dyn WasmStorage> {
        Box::new(self.clone())
    }
...
fn execute_transaction(
    ...
    vm_executor: &Arc<dyn crate::VMExecutor + Send + Sync>,
) -> TransactionReceipt {
    ...
    match &tx.payload {
        ...
        TxPayload::ContractDeploy { wasm_code, init_args } => {
            // compute address deterministically from tx hash (same canonical logic used in node and RPC path)
            let tx_hash = tx.hash();
            let mut address = [0u8; 20];
            address.copy_from_slice(&tx_hash[0..20]);

            // execute deployment via injected VM executor interface
            match vm_executor.execute(&wasm_code, init_args) {
                Ok(_) => {
                    let used = tx.gas_limit.min(1); // safe lower bound for this interface
                ...
                Err(err) => {
                    receipt.error = Some(format!("Contract deploy failed: {}", err));
                }
            }
        }
        TxPayload::ContractCall {
            ...
            let bytecode = if let Some((_, wasm_code)) = overlay
                .registry_updates
                .iter()
                .find(|(addr, _)| addr == contract_address)
            {
                wasm_code.clone()
            } else if let Some(contract) = base_registry.get_contract(contract_address) {
                contract.bytecode
            } else {
            ...
            // rely on symbolic method+args mapping via VM executor interface
            match vm_executor.execute(&bytecode, &args) {
                Ok(_) => {
                    let used = tx.gas_limit.min(1);
                    if let Err(e) = charge_fee(&mut sender_acc, used) {
                        receipt.error = Some(e);
                        return receipt;
                    }
                ...
                Err(err) => {
                    receipt.error = Some(format!("Contract call failed: {}", err));
                }
            }
        }
        TxPayload::Coinbase { .. } => {
            receipt.error =
                Some("Coinbase transactions cannot be executed in parallel executor".to_string());
        }
    }
    ...
    for batch in batches {
        ...
        let results: Vec<(TxId, Overlay, TransactionReceipt)> = batch
            ...
            .map(|tx| {
                ...
                let receipt = execute_transaction(tx, &mut overlay, state, registry, &vm_executor);
...
mod tests {
    ...
    struct DummyVMExecutor;

    impl crate::VMExecutor for DummyVMExecutor {
        fn execute(&self, _code: &[u8], _args: &[u8]) -> Result<Vec<u8>, String> {
            Ok(vec![])
        }
    }
    ...
    fn funded_account(state: &mut StateManager, addr: Address, amount: u64) {
        state
            .state_tree
            .update_account(addr, Account::new(amount, 0));
    ...
    fn independent_transfers_parallel() {
        ...
        let addr1: Address = Sha256::digest(&pubk1.serialize()[1..])[12..32]
            .try_into()
            .unwrap();

        let privk2 = SecretKey::new(&mut OsRng);
        let pubk2 = privk2.public_key(&secp);
        let addr2: Address = Sha256::digest(&pubk2.serialize()[1..])[12..32]
            .try_into()
            .unwrap();
        ...
        let tx1 = sign_tx(
            Transaction::new_transfer(addr1, rec1, 100, 0, 21000, 1),
            &privk1,
        );
        let tx2 = sign_tx(
            Transaction::new_transfer(addr2, rec2, 200, 0, 21000, 1),
            &privk2,
        );
    ...
    fn conflicting_transfers_sequential() {
        ...
        let addr: Address = Sha256::digest(&pubk.serialize()[1..])[12..32]
            .try_into()
            .unwrap();
        ...
        let tx1 = sign_tx(
            Transaction::new_transfer(addr, rec_a, 100, 0, 21000, 1),
            &privk,
        );
        // second tx should use the next nonce to be valid after tx1
        let tx2 = sign_tx(
            Transaction::new_transfer(addr, rec_b, 200, 1, 21000, 1),
            &privk,
        );
        let receipts = execute_block_transactions_parallel(
            vec![tx1.clone(), tx2.clone()],
    ...
    fn contract_call_executes() {
        ...
        let deploy_tx = sign_tx(
            Transaction::new_deploy(addr, wasm.clone(), vec![], 0, 100000, 1),
            &privk,
        );
        let call_tx = sign_tx(
            Transaction::new_call(
                addr,
                deploy_tx.hash()[0..20].try_into().unwrap(),
                "foo".to_string(),
                vec![],
                1,
                100000,
        ...
        let receipts = execute_block_transactions_parallel(
            vec![deploy_tx.clone(), call_tx.clone()],
            &mut state,
            &mut registry,
            storage.clone(),
            Arc::new(DummyVMExecutor),
    ...
    fn gas_accounting_and_overlay() {
        ...
        let tx = sign_tx(
            Transaction::new_transfer(addr, random_address(), 100, 0, 21000, 2),
            &privk,
        );
        let receipts = execute_block_transactions_parallel(
            vec![tx.clone()],
