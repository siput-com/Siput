...
impl BlockStore {
    ...
    /// use `new_with_path`.
    pub fn new() -> Self {
        // Use isolated paths for testing to avoid RocksDB lock conflicts
        // between simultaneously running tests and reused default data path.
        let default_path = if cfg!(test) || std::env::var("RUST_TEST_THREADS").is_ok() {
            let thread_id = format!("{:?}", std::thread::current().id());
            format!(
                "./target/test_data/block_store_test_{}_{}",
                std::process::id(),
                thread_id
            )
        } else {
        ...
        Self::new_with_path(&default_path).expect("Failed to create default BlockStore")
    }
...
mod block_store_tests {
    ...
    fn test_rollback_blocks_restores_state_and_mempool() {
        ...
        tx1.sign(&secp256k1::SecretKey::from_slice(&[1; 32]).unwrap())
            .unwrap();
        ...
        tx2.sign(&secp256k1::SecretKey::from_slice(&[1; 32]).unwrap())
            .unwrap();
