...
impl RollingWindowPruner {
    ...
    pub fn maybe_prune(
        ...
        info!(
            "Pruning {} blocks from height {} to {}",
            blocks_to_prune.len(),
            metadata.last_pruned_height + 1,
            prune_up_to
        );
        ...
        info!(
            "Pruning completed: {} blocks removed, {} bytes freed",
            blocks_to_prune.len(),
            total_size_pruned
        );
    ...
    pub fn take_state_snapshot(&self, height: u64, state: &mut StateManager) -> Result<(), String> {
        ...
        let snapshots_cf = self
            .db
            .cf_handle(STATE_SNAPSHOTS_CF)
            .ok_or("State snapshots column family not found")?;

        self.db
            .put_cf(&snapshots_cf, snapshot_key.as_bytes(), &snapshot_data)
        ...
        info!(
            "State snapshot taken at height {}: root={}, supply={}, accounts={}",
            height,
    ...
    fn save_metadata(&self, metadata: &PruningMetadata) -> Result<(), String> {
        ...
        self.db
            .put_cf(&meta_cf, b"metadata", &metadata_data)
    ...
    pub fn compact(&self) -> Result<(), String> {
        ...
        self.db
            .compact_range_cf(&snapshots_cf, None::<&[u8]>, None::<&[u8]>);
        self.db
