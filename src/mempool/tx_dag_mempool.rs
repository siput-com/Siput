...
impl MempoolTransaction {
    pub fn new(transaction: Transaction, parent_tx_hashes: Vec<[u8; 32]>, timestamp: u64) -> Self {
...
impl TxDagMempool {
    ...
    pub fn add_transaction(
        ...
        let tx_hash = tx.hash();
        let tx_hash_array: [u8; 32] = tx_hash
            .try_into()
        ...
        let is_ready = parent_tx_hashes.is_empty()
            || parent_tx_hashes
                .iter()
                .all(|parent_hash| !txs.contains_key(parent_hash));
    ...
    pub fn get_ready_transactions(&self) -> Vec<MempoolTransaction> {
        ...
        txs.values().filter(|tx| tx.is_ready).cloned().collect()
    }
            ...
            if let Some(children) = parent_child.get(tx_hash) {
                for child_hash in children {
                    ...
                    if let Some(child) = txs.get_mut(child_hash) {
                        ...
                        let _ = child;
                        let all_dependencies_fulfilled = deps.iter().all(|p| !txs.contains_key(p));
...
mod tests {
    ...
    fn test_add_transaction() {
        ...
        state
            .lock()
            .state_tree
            .update_account(from, crate::state::state_tree::Account::new(1000, 0));
        let mempool = TxDagMempool::new(1000, state.clone(), DEFAULT_MIN_GAS_PRICE);
        let to = [2u8; 20];
    ...
    fn test_get_ready_transactions() {
        ...
        let tx = create_test_transaction(to, 100, 0, &secret_key);
        mempool
            .add_transaction(tx, vec![], Some("peer2".to_string()))
            .unwrap();
    ...
    fn test_remove_transaction() {
        ...
        let tx = create_test_transaction(to, 100, 0, &secret_key);
        mempool
            .add_transaction(tx.clone(), vec![], Some("src".to_string()))
            .unwrap();
        assert_eq!(mempool.size(), 1);
    ...
    fn test_drop_low_fee_when_full() {
        ...
        tx1.gas_price = 10;
        mempool
            .add_transaction(tx1.clone(), vec![], Some("peer6".to_string()))
            .unwrap();
        let mut tx2 = create_test_transaction(to, 1, 1, &secret_key);
        tx2.gas_price = 20;
        // should drop tx1 and accept tx2
        mempool
            .add_transaction(tx2.clone(), vec![], Some("peer6".to_string()))
            .unwrap();
        assert_eq!(mempool.size(), 1);
