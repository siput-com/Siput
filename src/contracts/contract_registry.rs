...
impl ContractRegistry {
    ...
    pub fn new(db_path: &str) -> Result<Self, String> {
        ...
        Ok(ContractRegistry { db: Arc::new(db) })
    }
    ...
    pub fn get_contract(&self, address: &[u8; 20]) -> Option<ContractInfo> {
        ...
        match self.db.get(&key) {
            Ok(Some(data)) => match bincode::deserialize::<ContractInfo>(&data) {
                Ok(info) => {
...
mod tests {
    ...
    fn test_register_and_retrieve_contract() {
        ...
        reg.register_contract(address, bytecode.clone())
    ...
    fn test_persistence_after_restart() {
        ...
        // First session: register contract
        {
            ...
            registry
                .register_contract(address, bytecode.clone())
                .expect("Should register contract");
            registry
                .add_metadata(address, metadata.clone())
        ...
        {
            let registry = ContractRegistry::new(db_path).expect("Should open existing DB");
            ...
            assert_eq!(
                ...
                "Bytecode should match after restart"
            );
            assert_eq!(
                info.metadata, metadata,
                "Metadata should match after restart"
            );
    ...
    fn test_multiple_contracts_persistence() {
...

        {
            ...
            for (addr, bytecode) in contracts.iter() {
                registry
                    .register_contract(*addr, bytecode.clone())
                    .expect("Should register contract");
            }
        ...
        // Verify after restart
        {
            ...
            let count = registry.contract_count().expect("Should get count");
    ...
    fn test_contract_existence_check() {
        ...
        let address = [6u8; 20];
        assert!(
            !registry.contract_exists(&address),
            "Contract should not exist initially"
        );
        ...
        assert!(
            registry.contract_exists(&address),
            "Contract should exist after register"
        );
    ...
    fn test_metadata_update_persistence() {
...

        {
            ...
            registry
                ...
                .expect("Should register");
            registry
                .add_metadata(address, metadata1.clone())
                .expect("Should add metadata");
        ...
        // Update metadata
        {
            let mut registry = ContractRegistry::new(db_path).expect("Should open existing DB");

            registry
                .add_metadata(address, metadata2.clone())
        ...
        // Verify updated metadata persists
        {
            ...
            let contract = registry.get_contract(&address).unwrap();
            assert_eq!(
                contract.metadata, metadata2,
                "Updated metadata should persist"
            );
        }
    ...
    fn test_delete_contract() {
        ...
        registry.delete_contract(&address).expect("Should delete");
