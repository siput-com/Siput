...
impl BlockDAG {
    ...
    /// Load DAG index (tips and parent->children mapping) from persisted blocks.
    pub fn load_from_store(&mut self) {
        self.index = crate::dag::dag_index::DAGIndex::build_from_store(&self.store);
    }

    /// Create automatic genesis block
    pub fn create_genesis_block(&mut self) -> Block {
        // Genesis address: spt81ae155e79039d325d94d71c415b5961f87147fc
        let genesis_address: [u8; 20] = [
            0x81, 0xae, 0x15, 0x5e, 0x79, 0x03, 0x9d, 0x32, 0x5d, 0x94, 0xd7, 0x1c, 0x41, 0x5b,
            0x59, 0x61, 0xf8, 0x71, 0x47, 0xfc,
        ];
    ...
    pub fn insert_block(&mut self, block: Block) -> Result<(), String> {
        ...
        annotated_block.set_consensus_metadata(
            selected_parent,
            blue_score,
            chain_height,
            topo_index,
        ...
        for block in blocks {
            match self.insert_block(block) {
                ...
                Err(e) => {
                    ...
                    return Err(format!(
                        "Failed to insert block {}: {}. Successfully inserted {} blocks",
                        inserted, e, inserted
                    ));
                }
            }
    ...
    pub fn is_ancestor(&self, ancestor_hash: &BlockHash, block_hash: &BlockHash) -> bool {
        self.index
            .is_ancestor(ancestor_hash, block_hash, &self.store)
    }
    ...
    pub fn get_block_parents(&self, hash: &BlockHash) -> Option<Vec<BlockHash>> {
        self.get_block(hash)
            .map(|block| block.header.parent_hashes.clone())
    }
...
mod tests {
    ...
    fn test_topological_order() {
        ...
        assert!(
            order.iter().position(|h| h == &genesis.hash)
                < order.iter().position(|h| h == &child.hash)
        );
    }
