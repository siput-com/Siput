...
use crate::state::state_tree::{Account, SparseMerkleTree};
...
impl StateManager {
    pub fn new() -> Self {
        ...
        state_tree.update_account(
            EMISSION_STATE_ACCOUNT,
            Account::new(crate::consensus::mining::CTS_GENESIS_ALLOCATION, 0),
        );
    ...
    pub fn initialize_tokenomics(&mut self) {
        ...
        if self.state_tree.get_account(&GENESIS_WALLET).is_none() {
            self.state_tree
                .update_account(GENESIS_WALLET, Account::new(0, 0));
        }
        let genesis_account = self
            .state_tree
            .get_account(&GENESIS_WALLET)
            .cloned()
            .unwrap();
        if genesis_account.balance == 0 {
            self.state_tree.update_account(
                GENESIS_WALLET,
                Account::new(100_000_000, genesis_account.nonce),
            );
        }
        ...
        {
            self.state_tree
                .update_account(EMISSION_STATE_ACCOUNT, Account::new(0, 0));
        }
    ...
    pub fn apply_transaction(
        ...
        match &tx.payload {
            ...
            TxPayload::Coinbase {
                ...
                // Mint new supply to miner and track emitted supply
                let miner_account = self
                    .state_tree
                    .get_account(to)
                    .cloned()
                    .unwrap_or(Account::new(0, 0));
                let new_balance = miner_account.balance.saturating_add(*amount);
                self.state_tree
                    .update_account(*to, Account::new(new_balance, miner_account.nonce));

                // Track emitted supply for total supply calculations
                self.add_emitted_supply(*amount);
                self.current_state_root = self.state_tree.calculate_root();
                Ok(())
            }
            TxPayload::ContractDeploy { .. } => {
                // Contract deployment handled by ContractExecutor
                Ok(())
            }
            TxPayload::ContractCall { .. } => {
                // Contract call handled by ContractExecutor
                Ok(())
            }
        }
    }
    ...
    fn apply_transfer(
        &mut self,
        from: crate::core::transaction::Address,
        to: crate::core::transaction::Address,
        amount: u64,
        nonce: u64,
        fee: u64,
    ) -> Result<(), String> {
    ...
    pub fn apply_block(
        &mut self,
        _transactions: &[crate::core::transaction::Transaction],
    ) -> Result<(), String> {
        Ok(())
    }
    ...
    pub fn snapshot_and_prune(
        ...
        let _ = pruner.maybe_prune(current_height, self);
    }
...
mod tests {
    ...
    fn test_apply_transfer_with_fee() {
        ...
        mgr.state_tree
            .update_account(sender, crate::state::state_tree::Account::new(1000, 0));
        ...
        // give enough balance
        mgr.state_tree
            .update_account(sender, crate::state::state_tree::Account::new(50000, 0));
    ...
    fn test_fee_and_credit() {
        ...
        mgr.state_tree
            .update_account(addr, crate::state::state_tree::Account::new(1000, 0));
