use crate::utils::calculations::{calculate_expected_block_reward, RewardConfig};
use crate::consensus::{DaaConfig, DifficultyState};
...
impl BlockProducer {
    ...
    pub fn new(
        ...
        Self {
            ...
            min_transactions,
            max_transactions,
            difficulty_state: Arc::new(RwLock::new(DifficultyState::new())),
        }
    }
    ...
    pub fn create_block(
        ...
        ready_txs
            .sort_by_key(|mt| std::u64::MAX - mt.transaction.gas_price.saturating_sub(base_fee));
        ...
        // Determine chain progress (next height), recent timestamps, and parent blocks for reward calculation
        let (chain_progress, recent_timestamps, parent_hashes) = {
            let dag_guard = self.dag.read();
            // Use current tip height + 1 to represent the height of the block being produced.
            let tip_height = dag_guard
                .get_tip_blocks()
                .iter()
                .map(|b| b.header.chain_height)
                .max()
                .unwrap_or(0);
            let chain_progress = (tip_height + 1) as f64;
            let timestamps =
                dag_guard.get_recent_timestamps(RewardConfig::default().activity_window);
            let tip_hashes = dag_guard.get_tips();
            let genesis_hash = dag_guard.get_topological_order().iter().find_map(|h| {
                dag_guard
                    .get_block(h)
            ...
            let parents = crate::consensus::select_mining_parents(&tip_hashes, genesis_hash);
            (chain_progress, timestamps, parents)
        };
        ...
        // Set block transactions (coinbase reward is tracked in block.reward)
        ...
        // Override block reward with the calculated expected reward
        block.reward = reward;
        // Recompute hash to keep it consistent after mutating the block
    ...
    /// Create a candidate block using a precomputed DAG snapshot (parents + timestamps).
    ///
    /// This avoids holding a DAG lock while building the block template.
    pub fn create_block_with_snapshot(
        &self,
        difficulty: u32,
        base_fee: u64,
        _state_root: [u8; 32],
        _chain_progress: f64,
        _recent_timestamps: Vec<u64>,
        parent_hashes: Vec<BlockHash>,
        _genesis_hash: Option<BlockHash>,
    ) -> Result<Block, String> {
        // Ambil ready transactions
        let mut ready_txs = (*self.mempool).get_ready_transactions();
        ...
        // Determine emitted supply from persistent state
        let _emitted_supply = self.state_manager.lock().get_emitted_supply();

        // Calculate mining reward using the current reward model
        let reward = calculate_expected_block_reward(0, 0.0, &[], transactions.len(), &RewardConfig::default());
    ...
    pub fn mine_block(
        ...
        let meets = |b: &Block| {
            crate::consensus::meets_difficulty(&b.calculate_hash(), b.header.difficulty)
        };

        // search in parallel
        use rayon::prelude::*;

        if let Some(nonce) = (0u64..u64::MAX).into_par_iter().find_any(|&n| {
...
mod tests {
    ...
    fn test_create_block_with_transactions() {
        ...
        state
            .lock()
            .state_tree
            .update_account(from, crate::state::state_tree::Account::new(10_000, 0));
    ...
    fn test_create_block_respects_max_transactions() {
        ...
        state
            .lock()
            .state_tree
            .update_account(sender, crate::state::state_tree::Account::new(1_000_000, 0));
        for i in 0..10 {
            let from = sender;
            let to = [2u8; 20];
