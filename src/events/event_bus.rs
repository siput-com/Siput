        ...
        tokio::spawn(async move {
            loop {
                ...
                let select_all = futures::future::select_all(
                    all_receivers
                        .into_iter()
                        .map(|mut rx| Box::pin(async move { rx.recv().await })),
                );
...
mod tests {
    use super::*;
    use crate::events::event_types::{Event, TransactionStatus};
    ...
    async fn test_event_bus_publish_subscribe() {
        ...
        let event = Event::new_block(crate::core::Block::default(), 1, "test_node".to_string());

        bus.publish(event.clone()).await;

        // Receive the event
        match receiver.recv().await {
            Ok(received_event) => assert_eq!(received_event.id, event.id),
