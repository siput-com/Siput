...
/// Siput Blockchain CLI
#[derive(Parser)]
#[command(name = "spt")]
#[command(about = "Siput blockchain node and wallet CLI")]
pub struct Cli {
    #[command(subcommand)]
...
impl CliHandler {
    ...
    pub async fn handle_node_start(
        ...
        let _rpc_node = node_arc.clone();
        let _rpc_addr = rpc_addr.to_string(); // Convert to owned String for 'static lifetime
                                             // TODO: Fix Send issue
    ...
    pub async fn handle_wallet_create(
        ...
        if let Some(path) = output {
            ...
            println!(
                "Private Key: {}",
                hex::encode(wallet.private_key.secret_bytes())
            );
    ...
    pub async fn handle_wallet_balance(
        ...
        if response.status().is_success() {
            ...
            println!("Balance: {} spt", balance["balance"]);
    ...
    pub async fn handle_tx_send(
        ...
        // Parse recipient address (expect spt... format)
        if !to.starts_with("spt") {
            return Err("Invalid address format. Must start with 'spt'".into());
        }
