...
impl CLIInterface {
    ...
    pub async fn run(&self) -> Result<(), String> {
        println!("=== Siput Production CLI ===");
        println!(
            "Node uptime: {} seconds",
            self.metrics.read().uptime_seconds()
        );
        println!(
            "Transactions processed: {}",
            self.metrics.read().transactions_processed
        );
    ...
    async fn cmd_send_transaction(&self) -> Result<(), String> {
        ...
        let secret_key =
            SecretKey::from_slice(&[1; 32]).map_err(|_| "Failed to create secret key")?;
        tx.sign(&secret_key)?;
    ...
    fn cmd_print_finality(&self) -> Result<(), String> {
        ...
        println!(
            "Confirmation depth: {}",
            node.finality_engine.confirmation_depth
        );
    ...
    async fn cmd_prune_storage(&self) -> Result<(), String> {
        ...
        let pruned_blocks = node
            .prune_storage(window)
            .await
...
mod tests {
    ...
    fn test_address_parsing() {
        ...
        let addr: [u8; 20] = hex::decode(addr_str).unwrap().try_into().unwrap();
