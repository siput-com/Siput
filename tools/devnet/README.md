...
Local development network tools for Siput blockchain development.

## Overview

The devnet tools provide a complete local development environment for building and testing Siput applications. It includes commands to start/stop/reset a local network, spawn test nodes, and manage test accounts.
...
The devnet works seamlessly with the Siput SDK:

```rust
use siput_core::sdk::Client;
...
- `SIPUT_DEVNET_PORT` - Override default RPC port
- `SIPUT_DEVNET_DATA_DIR` - Custom data directory
- `SIPUT_DEVNET_LOG_LEVEL` - Set log verbosity
