...
# Start devnet
start_devnet() {
    log_info "Starting Siput devnet..."

    # Check if already running
    if pgrep -f "siput.*devnet" > /dev/null; then
        log_warning "Devnet appears to already be running"
        return 1
    fi

    # Build the project if needed
    if [ ! -f "$PROJECT_ROOT/target/release/siput" ]; then
        log_info "Building Siput..."
        cd "$PROJECT_ROOT"
        cargo build --release
    fi
    ...
    # Start the node
    cd "$PROJECT_ROOT"
    nohup ./target/release/siput --config "$CONFIG_FILE" > "$DEVNET_DIR/node.log" 2>&1 &
...
EOF
        ...
        # Start the node
        cd "$PROJECT_ROOT"
        nohup ./target/release/siput --config "$config_file" > "$DEVNET_DIR/node$i.log" 2>&1 &
