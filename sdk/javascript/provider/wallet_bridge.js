...
class SiputWalletBridge {
    ...
    async connectWallet() {
        ...
        // Check for Siput wallet extension
        if (!window.siputWallet) {
            throw new Error('Siput wallet extension not found. Please install the Siput wallet.');
        }

        try {
            // Request wallet connection
            const result = await window.siputWallet.connect();
            this.connectedWallet = window.siputWallet;
...
function getWalletBridge(provider = null) {
    if (!globalWalletBridge) {
        ...
        globalWalletBridge = new SiputWalletBridge(provider);
    }
...
if (typeof module !== 'undefined' && module.exports) {
    // Node.js
    module.exports = { SiputWalletBridge, getWalletBridge };
} else if (typeof define === 'function' && define.amd) {
    // AMD
    define([], function() { return { SiputWalletBridge, getWalletBridge }; });
} else if (typeof window !== 'undefined') {
    // Browser global
    window.SiputWalletBridge = SiputWalletBridge;
    window.getSiputWalletBridge = getWalletBridge;
}