...
use siput_core::core::transaction::TxPayload;
use siput_core::{Address, Transaction};

/// Builder for creating Siput transactions.
...
impl TransactionBuilder {
    ...
    pub fn deploy_contract(mut self, wasm_code: Vec<u8>, init_args: Vec<u8>) -> Self {
        self.payload = Some(TxPayload::ContractDeploy {
            wasm_code,
            init_args,
        });
        self
    ...
    pub fn call_contract(
        ...
        self.payload = Some(TxPayload::ContractCall {
            contract_address,
            method,
            args,
        });
    ...
    /// Getters for internal fields to support higher-level SDK operations.
    ...
    pub fn build(self) -> Result<Transaction, SdkError> {
        let from = self
            .from
            .ok_or_else(|| SdkError::TransactionError("Missing from address".into()))?;
        let payload = self
            .payload
            .ok_or_else(|| SdkError::TransactionError("Missing payload".into()))?;
        ...
        let tx = match payload {
            ...
            TxPayload::ContractDeploy {
                wasm_code,
                init_args,
            } => Transaction::new_deploy(from, wasm_code, init_args, nonce, gas_limit, gas_price),
            ...
            TxPayload::Coinbase { .. } => {
                return Err(SdkError::TransactionError(
                    "Coinbase transactions cannot be built by TransactionBuilder".into(),
                ))
            }
        };
