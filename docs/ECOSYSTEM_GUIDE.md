# Siput Ecosystem Guide

Welcome to the Siput blockchain ecosystem! This comprehensive guide covers everything you need to build applications on Siput, from wallets and dApps to explorers and analytics tools.
...
Siput provides a complete developer ecosystem:
...
```rust
use siput_core::sdk::{Wallet, Client, TransactionBuilder};
...
```javascript
// Connect to Siput
const provider = new SiputProvider('http://localhost:8080');

// Create wallet
const wallet = SiputWallet.createWallet();
console.log('Address:', wallet.address);

// Connect wallet extension (if available)
const walletBridge = new SiputWalletBridge(provider);
...
```rust
use siput_core::sdk::mobile::{MobileClient, MobileWallet};
...
<head>
    ...
    <script src="siput-provider.js"></script>
    <script src="siput-events.js"></script>
    <script src="siput-wallet-bridge.js"></script>
</head>
<body>
    <h1>My Siput dApp</h1>
    ...
    <script>
        const provider = new SiputProvider('http://localhost:8080');
        const walletBridge = new SiputWalletBridge(provider);
        const events = new SiputEvents(provider);
...
```rust
use siput_core::sdk::{Client, EventBus, EventListener, EventHandler};
use std::sync::Arc;
...
impl EventHandler for MyDApp {
    async fn on_new_block(&self, event: siput_core::sdk::events::Event) {
        let count = self.event_count.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        println!("Block #{}: {:?}", count, event);
    }

    async fn on_new_transaction(&self, event: siput_core::sdk::events::Event) {
        println!("New transaction: {:?}", event);
    }

    async fn on_contract_event(&self, event: siput_core::sdk::events::Event) {
        println!("Contract event: {:?}", event);
    }

    async fn on_peer_event(&self, event: siput_core::sdk::events::Event) {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _subscriptions = siput_core::sdk::events::create_listener_from_handler(
        event_bus,
        "my_dapp".to_string(),
        dapp,
    ).await;
    ...
    let test_event = siput_core::sdk::events::Event::new_block(
        siput_core::core::Block::default(),
        1,
        "test".to_string(),
    );
...
Siput uses WebAssembly (WASM) for smart contracts. Here's a simple token contract:

```rust
// contracts/token.rs
use siput_core::vm::host_functions::*;
...
```rust
use siput_core::sdk::{Client, Wallet, ContractClient, TransactionBuilder};
...
```rust
use siput_core::sdk::ContractClient;
...
```rust
use siput_core::sdk::{EventBus, EventListener, EventHandler};
use std::sync::Arc;
...
async fn event_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let subscriptions = siput_core::sdk::events::create_listener_from_handler(
        event_bus.clone(),
        "my_app".to_string(),
        processor,
    ).await;
...
ws.onopen = () => {
    console.log('Connected to Siput events');
...
```rust
use siput_core::indexer::{BlockIndexer, TransactionIndexer, AddressIndexer};
...
async fn explorer_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let block = siput_core::core::Block::default();
...
```rust
use siput_core::indexer::TransactionIndexer;
...
```rust
use siput_core::indexer::BlockIndexer;
...
async fn mobile_app_logic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client = MobileClient::new("http://api.siput.com".to_string());
...
use siput_core::sdk::mobile::MobileClient;
...
- **Community**: Join Siput Discord/Forum
...
**Happy building on Siput!** 🚀

The ecosystem is designed to be developer-friendly, secure, and scalable. Whether you're building a simple wallet or a complex dApp, Siput provides the tools and infrastructure you need.