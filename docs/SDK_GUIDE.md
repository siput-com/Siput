...
This guide shows how to use the built-in **Siput SDK** to interact with a running node over the RPC API.

> The SDK is part of the `siput-core` crate and is available via `siput_core::sdk` once you add `siput-core` as a dependency.

---

## 1) Setup

Add `siput-core` to your `Cargo.toml`:

```toml
[dependencies]
siput-core = { path = "../" } # or a published version
```

Then in your code:

```rust
use siput_core::sdk::{Client, Wallet, TransactionBuilder};
use siput_core::core::Address;
...
println!("Address: spt{}", hex::encode(wallet.address));
```
...
Use `siput_core::sdk::network::Network` when you need higher-level helpers like waiting for a tip height.

```rust
use siput_core::sdk::network::Network;
...
- Address encoding uses the `spt<hex>` prefix that the node expects.
